...
#define TEST_FIXING_ALL_HYPO_TO_FIRST 0
#define TEST_PERTURB_ALL_HYPOS 0
#define TEST_FIXING_HYPO_DEPTH 1
        if (TEST_FIXING_ALL_HYPO_TO_FIRST) {
            sprintf(MsgStr, "WARNING: TEXT_FIXING_ALL_HYPO_TO_FIRST - only use for testing!!!!!!");
            nll_putmsg(0, MsgStr);
            Hypos[nHypos].dlat = Hypos[0].dlat;
            Hypos[nHypos].dlong = Hypos[0].dlong;
            Hypos[nHypos].depth = Hypos[0].depth;
            latlon2rect(0, Hypos[nHypos].dlat, Hypos[nHypos].dlong, &(Hypos[nHypos].x), &(Hypos[nHypos].y));
            Hypos[nHypos].z = Hypos[nHypos].depth;
            Hypos[nHypos].dotime = 0.0;
        }
        if (TEST_PERTURB_ALL_HYPOS) {
            ...
            nll_putmsg(0, MsgStr);
            Hypos[nHypos].dlat += get_rand_double(-ds * KM2DEG, ds * KM2DEG);
            Hypos[nHypos].dlong += get_rand_double(-ds * KM2DEG, ds * KM2DEG);
            Hypos[nHypos].depth += get_rand_double(-ds, ds);
            latlon2rect(0, Hypos[nHypos].dlat, Hypos[nHypos].dlong, &(Hypos[nHypos].x), &(Hypos[nHypos].y));
            Hypos[nHypos].z = Hypos[nHypos].depth;
            Hypos[nHypos].dotime = 0.0;
        }
        if (TEST_FIXING_HYPO_DEPTH) {
            ...
            nll_putmsg(0, MsgStr);
            // Hypos[nHypos].depth = 17.0;
            Hypos[nHypos].depth = 0.5;
            latlon2rect(0, Hypos[nHypos].dlat, Hypos[nHypos].dlong, &(Hypos[nHypos].x), &(Hypos[nHypos].y));
            Hypos[nHypos].z = Hypos[nHypos].depth;
            Hypos[nHypos].dotime = 0.0;
        }
...
void DiffLocUpdateAfterAccept(double misfit, double value, double dlike, int reverse_comparison, double xval, double yval, double zval, double tval,
    ...
    // 20180620 AJL
#define TEST_AUTO_UPDATE_MetStartSave 0
...
int DiffLocMetropolis(int num_arr_total, int num_arr_loc,
    ...
    double probCommonMoveAllHypos = 0.0 / (double) NumHypocenters;
