...
/*
        ...
        2020-04-12 pz Add an option MAPLINE GMT_YX_K for Krovak map coordinates.
        2020-04-13 pz Create a bash script instead of a csh script.
...
int GenGMTCommands(char cplotmode, char cdatatype,
    ...
    fprintf(fp_gmt, "#!/bin/bash\n#\n\n");
//    fprintf(fp_gmt, "source gmt4.sh\n\n");

    // gmtdefaults
    fprintf(fp_gmt, "gmt set  PS_PAGE_ORIENTATION portrait  MAP_ORIGIN_X 0.5  MAP_ORIGIN_Y 0.5 \n\n");
    fprintf(fp_gmt, "gmt set  FONT_ANNOT_PRIMARY 8  FONT_ANNOT_SECONDARY 6  FONT_TITLE 12 FONT_LABEL 10\n\n");
    fprintf(fp_gmt, "gmt set  MAP_LABEL_OFFSET 0.1c  MAP_ANNOT_OFFSET_PRIMARY 0.1c MAP_ANNOT_OFFSET_SECONDARY 0.1c\n\n");

    fprintf(fp_gmt, "POSTSCRIPT_NAME=%s\n\n", fn_root_output);

    fprintf(fp_gmt, "if [ -f $POSTSCRIPT_NAME.ps ] ;then rm $POSTSCRIPT_NAME.ps; fi\n\n");
    ...
    if (!plot_lat_lon)
        fprintf(fp_gmt, "PLOT_LAT_LONG=false\n");
    else
        fprintf(fp_gmt, "#PLOT_LAT_LONG=false\n");
    //if (doLatLong) {
    fprintf(fp_gmt,
    ...
    if (plot_lat_lon)
        fprintf(fp_gmt, "PLOT_LAT_LONG=true\n");
    else
        fprintf(fp_gmt, "#PLOT_LAT_LONG=true\n");
    //}
    fprintf(fp_gmt,
    ...
    if (fp_hypo != NULL && ((ihypo = GetHypLoc(fp_hypo, NULL, &Hypo, Arrival,
        ...
        fprintf(fp_gmt,
                "gmt pstext -R0.5/1.0/0.5/1.0 -Bf10 -JX%lf/%lf -F+f+a+j -K -O << END >> %s.ps\n%lf %lf %dp,%d %d %d %s\nEND\n\n",
                PLOT_WIDTH, PLOT_HEIGHT, fn_ps_output,
                0.75, 0.98, TITLE_FONT_SIZE, TITLE_FONT, 0, 2, Hypo.comment);
        ...
        fprintf(fp_gmt,
                "gmt pstext -R0.5/1.0/0.5/1.0 -Bf10 -JX%lf/%lf -F+f+a+j -K -O << END >> %s.ps\n%lf %lf %dp,%d %d %d %s\nEND\n\n",
                PLOT_WIDTH, PLOT_HEIGHT, fn_ps_output,
                0.75, 0.97, HYPO_FONT_SIZE, HYPO_FONT, 0, 2, hypotext);
    ...
    if (PLOT_LEGEND_LINE) {
        ...
        if (cdatatype == 'S') {
            fprintf(fp_gmt,
                    "gmt pstext -R0.5/1.0/0.5/1.0 -Bf10 -JX%lf/%lf -F+f+a+j -K -O << END >> %s.ps\n%lf %lf %dp,%d %d %d %s\nEND\n\n",
                    PLOT_WIDTH, PLOT_HEIGHT, fn_ps_output,
                    0.75, 0.96, HYPO_FONT_SIZE, TITLE_FONT, 0, 2, "PDF scatter sample");
        } else if (cdatatype == 'E') {
            fprintf(fp_gmt,
                    "gmt pstext -R0.5/1.0/0.5/1.0 -Bf10 -JX%lf/%lf -F+f+a+j -K -O << END >> %s.ps\n%lf %lf %dp,%d %d %d %s\nEND\n\n",
                    PLOT_WIDTH, PLOT_HEIGHT, fn_ps_output,
                    0.75, 0.96, HYPO_FONT_SIZE, TITLE_FONT, 0, 2,
                    "Error Ellipsoid, Expectation (dot) and Maximum Likelihood (star)");
        } else if (cdatatype == 'M') {
            fprintf(fp_gmt,
                    "gmt pstext -R0.5/1.0/0.5/1.0 -Bf10 -JX%lf/%lf -F+f+a+j -K -O << END >> %s.ps\n%lf %lf %dp,%d %d %d %s\nEND\n\n",
                    PLOT_WIDTH, PLOT_HEIGHT, fn_ps_output,
                    0.75, 0.96, HYPO_FONT_SIZE, TITLE_FONT, 0, 2, "Double-couple focal mechanisms");
        } else if (cdatatype == 'R') {
            fprintf(fp_gmt,
                    "gmt pstext -R0.5/1.0/0.5/1.0 -Bf10 -JX%lf/%lf -F+f+a+j -K -O << END >> %s.ps\n%lf %lf %dp,%d %d %d %s %s\nEND\n\n",
                    PLOT_WIDTH, PLOT_HEIGHT, fn_ps_output,
                    0.75, 0.96, HYPO_FONT_SIZE, TITLE_FONT, 0, 2, arg_elements[0], "Residuals");
        } else if (cdatatype == 'C') {
            fprintf(fp_gmt,
                    "gmt pstext -R0.5/1.0/0.5/1.0 -Bf10 -JX%lf/%lf -F+f+a+j -K -O << END >> %s.ps\n%lf %lf %dp,%d %d %d %s %s\nEND\n\n",
                    PLOT_WIDTH, PLOT_HEIGHT, fn_ps_output,
                    0.75, 0.96, HYPO_FONT_SIZE, TITLE_FONT, 0, 2, arg_elements[0], "StaCorr");
        }
    ...
    if (cdatatype != 'S' && cdatatype != 'E' && cdatatype != 'M' && cdatatype != 'R' && cdatatype != 'C') {
        ...
        fprintf(fp_gmt,
                "gmt psscale -C%s.%scpt -D%lf/%lf/14.0/0.5h -B+l\"%s\" -X%lf -Y%lf -K -O >> %s.ps\n\n",
    ...
    if (cdatatype == 'R' || cdatatype == 'C') {
        ...
        if (cdatatype == 'R') {
            fprintf(fp_gmt,
                    "gmt pstext $JVAL $RVAL -N -F+f+a+j -K -O << END >> %s.ps\n%lf %lf %dp,%d %d %d %s Residuals      %s\nEND\n\n",
                    fn_ps_output, xres - 0.25, yres,
                    HYPO_FONT_SIZE, STA_FONT, 0, 7, arg_elements[0], res_legend_mag_string);
        } else {
            fprintf(fp_gmt,
                    "gmt pstext $JVAL $RVAL $BVAL -N  -F+f+a+j-K -O << END >> %s.ps\n%lf %lf %dp,%d %d %d %s StaCorr      %s\nEND\n\n",
                    fn_ps_output, xres - 0.25, yres,
                    HYPO_FONT_SIZE, STA_FONT, 0, 7, arg_elements[0], res_legend_mag_string);
        }
        // negative residual
        ...
        fprintf(fp_gmt,
                "gmt pstext $JVAL $RVAL $BVAL -N -F+f+a+j -K -O << END >> %s.ps\n%lf %lf %dp,%d %d %d %s\nEND\n\n",
                fn_ps_output, xres - 0.25, yres,
                HYPO_FONT_SIZE, STA_FONT, 0, 7, res_legend_mag_string);
    ...
    fprintf(fp_gmt,
            "gmt pstext -R0.5/1.0/0.5/1.0 -Bf10 -JX%lf/%lf -X%lf -Y%lf -F+f+a+j -O << END >> %s.ps\n%lf %lf %dp,%d %d %d %s\nEND\n\n",
            PLOT_WIDTH, PLOT_HEIGHT, -xshift_cum, -yshift_cum,
            fn_ps_output,
            0.501, 0.501, ANNOTATION_FONT_SIZE, ANNOTATION_FONT, 0, 1, signature_str);
...
//    fprintf(fp_gmt, "RVAL=\'-R%lf/%lf/%lf/%lf/-9999/9999\'\n",
    ...
    fprintf(fp_gmt, "BVAL=( "); 
    fprintf(fp_gmt, "-Bx%lf+l\"%s\" ", htick_int, horiz_label);
    fprintf(fp_gmt, "-By%lf+l\"%s\" ", vtick_int, vert_label);
    fprintf(fp_gmt, "-B%s+t\"%s\" )\n", chr_bounds, chr_title);
    ...
    *pylen = (vert_max - vert_min) * plot_scale;
    ...
    if (doLatLong)
        fprintf(fp_gmt, "if [ $PLOT_LAT_LONG = false ] ;then\n");
    fprintf(fp_gmt,
    ...
    if (doLatLong) {
        fprintf(fp_gmt, "if [ $PLOT_LAT_LONG = true ] ;then\n");
    ...
    if (cdatatype == 'G') {
        ...
        if (doLatLong)
            fprintf(fp_gmt, "if [ $PLOT_LAT_LONG = false ] ;then\n");
        fprintf(fp_gmt,
                 "gmt xyz2grd %s -G%sgmt -I%lf/%lf $RVAL -Dkm/km//1/0/%s -V -ZTLf\n",
        ...
        if (doLatLong) {
            ...
            fprintf(fp_gmt, "if [ $PLOT_LAT_LONG = true ] ;then\n");
            fprintf(fp_gmt,
                    "gmt xyz2grd %s -G%sgmt -I%lf/%lf ${RVAL} -Ddeg/deg//1/0/%s -V -ZTLf\n",
...
//        fprintf(fp_gmt, "SCALE_FLAG=\n\n");
        ...
        if (pgrid->type == GRID_PROB_DENSITY) {
            ...
            fprintf(fp_gmt,
                    "gmt grdimage -nb %sgmt -C%s.conf.cpt $JVAL $RVAL -K -O >> %s.ps\n\n",
                    fn_gmtgrd, fn_root_output, fn_ps_output);
            if (no_plot_contours) {
                fprintf(fp_gmt, "#");
            }
            fprintf(fp_gmt,
                    "gmt grdcontour %sgmt %s -C%s.conf.cpt -D%s $JVAL $RVAL -K -O >> %s.ps\n\n",
            ...
            fprintf(fp_gmt,
                    "gmt grdimage -nb %sgmt -C%s.misfit.cpt $JVAL $RVAL -K -O >> %s.ps\n\n",
                    fn_gmtgrd, fn_root_output, fn_ps_output);
            ...
            fprintf(fp_gmt,
                    "gmt grdcontour %sgmt %s -C%s.misfit.cpt $JVAL $RVAL -K -O >> %s.ps\n\n",
...
//                        "   SCALE_FLAG=\n");
                ...
                if (pgrid->type == GRID_LIKELIHOOD) { // likelihood color table
                    fprintf(fp_gmt,
                            "   gmt makecpt -Z -Chot -I -T0/1/0.1 > %s.cpt\n",
                            fn_root_output);
                } else { // rainbow
...
//                    "   SCALE_FLAG=-L\n");
            ...
            fprintf(fp_gmt, "gmt grdimage -nb %sgmt -C%s.cpt $JVAL $RVAL -K -O >> %s.ps\n\n",
                    fn_gmtgrd, fn_root_output, fn_ps_output);
            ...
            fprintf(fp_gmt,
                    "gmt grdcontour %sgmt %s -C%s.cpt $JVAL $RVAL -K -O >> %s.ps\n\n",
                    fn_gmtgrd, label_contours, fn_root_output, fn_ps_output);
    ...
    if (cdatatype == 'S' || plotExpScatter) {
        ...
        fprintf(fp_gmt, "gmt psxy %s $JVAL $RVAL -Wdefault,red -Sp -Gred -K -O >> %s.ps\n",
                fnscat, fn_ps_output);
    ...
    if (!(cdatatype == 'S' || cdatatype == 'E' || plotExpScatter)
        ...
        fprintf(fp_gmt,
                "gmt psxy $JVAL $RVAL -Wdefault,black,dotted -K -O << END >> %s.ps\n",
                fn_ps_output);
        fprintf(fp_gmt,
                ">\n%lf %lf\n%lf %lf\n>\n%lf %lf\n%lf %lf\nEND\n\n",
    ...
    if (cdatatype == 'M') {
        ...
        while (ReadFocalMech(&fpio_tmp, fnroot_input, &focalMech, Arrival, &NumArrivals) != EOF) {
            ...
            if ((istat = PlotFocalMechanism(file_id,
                    ...
                    fp_gmt, arg_elements[0], "-Wdefault,255/0/0 ", "-G255/0/0 ",
    ...
    if (/*cdatatype != 'E' &&*/ cviewmode == 'H') {
        if (NumStations>0) {
            fprintf(fp_gmt, "# Station\ngmt pstext $JVAL $RVAL -F+f%dp,%d,white=~thick,black+a%d+j%d -K -O << END >> %s.ps\n",
                    STA_FONT_SIZE, STA_FONT, 0, 6, fn_ps_output);
            ...
            for (nsta = 0; nsta < NumStations; nsta++) {
                ...
                fprintf(fp_gmt, "%lf %lf %s\n", stax, stay, psta->label);
            }
            fprintf(fp_gmt, "END\n\n");
        }
    } 
...
int grd2GMT(int nmapfile, double xmin0, double ymin0, double xmax0, double ymax0,
    ...
    fprintf(fp_gmt, "gmt grdimage -nb %s %s -C%s %s %s %s -K -O >> %s.ps\n",
...
int MapLines2GMT(int nmapfile, double xmin0, double ymin0, double xmax0, double ymax0,
        ...
        for (;;) {
            ...
            if (strcmp(mapfile[nmapfile].format,
                ...
                if (sscanf(line, "%lf %lf",
                        &maplong, &maplat) != 2)
                    break;
            } else if (strcmp(mapfile[nmapfile].format,
                    "XY_LONLAT") == 0) {
                sscanf(line, "%lf %lf",
                        &maplong, &maplat);
                if (maplong < -900.0) {
    ...
    if (doLatLong)
        fprintf(fp_gmt, "if [ $PLOT_LAT_LONG = false ] ;then\n");
    fprintf(fp_gmt,
            "gmt psxy %s $JVAL $RVAL -Wthin,%d/%d/%d -K -O >> %s.ps\n",
...
int PlotTraditionStats(char cdatatype, char view_type, double barlen,
    ...
    if (plt_code[0] == '1') {
        ...
        fprintf(fp_io,
                "# Maximum Likelihood\ngmt psxy $JVAL $RVAL -Wdefault,%s -Sa%lf -G%s -K -O << END >> %s.ps\n", GMTcolor, 0.4 * barlen / 2.54, GMTcolor, "${POSTSCRIPT_NAME}");
        if (view_type == 'Y')
    ...
    if (plt_code[1] == '1') {
        ...
        fprintf(fp_io,
                "# Expectation\ngmt psxy $JVAL $RVAL -Wdefault,%s -G0 -Sc%lf -K -O << END >> %s.ps\n",
    ...
    if (plt_code[2] == '1') {
        ...
        fprintf(fp_io,
                "# Error Ellipsoid\ngmt psxy $JVAL $RVAL -Wdefault,%s -K -O << END >> %s.ps\n>\n",
...
void genResidualGMT(FILE* fp_out, char* xtra_args, double resid, double x, double y, double scale) {
    ...
    if (resid_scaled >= 0.0) {
        ...
        fprintf(fp_out,
                "gmt psxy $JVAL $RVAL -Sc -W8,0/0/255 %s -K -O  << END >> ${POSTSCRIPT_NAME}.ps\n%lf %lf %lf\nEND\n",
                xtra_args, x, y, fabs(resid_plot));
    } else {
        resid_plot = resid_scaled > -RESID_MIN ? -RESID_MIN : resid_scaled;
        fprintf(fp_out,
                "gmt psxy $JVAL $RVAL -St -W8,255/0/0 %s -K -O  << END >> ${POSTSCRIPT_NAME}.ps\n%lf %lf %lf\nEND\n",
                xtra_args, x, y, fabs(resid_plot));
    }
}
