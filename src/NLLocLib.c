...
int MetLearn; /* learning length in number of samples for
...
int MetUse; /* number of samples to use
...
int GetNextObs(HypoDesc* phypo, FILE* fp_obs, ArrivalDesc *arrival, char* ftype_obs, int nfirst) {
    ...
    } else if (strcmp(ftype_obs, "HYPO3D") == 0) {
...
struct hyp_record rec;
        hypline(&rec, line);
          strcpy(arrival->label, rec.sta);
          strcpy(arrival->network, "?");
          strcpy(arrival->comp, "HHZ");
          strcpy(arrival->phase, rec.phase);
          arrival->year = rec.year;
          if (rec.year < 85) {
            arrival->year = rec.year+2000;
            }
          else {
            arrival->year = rec.year+1900;
            } 
          arrival->month = rec.month; 
          arrival->day = rec.day; 
          arrival->hour = rec.hour; 
          arrival->min = rec.minute; 
          arrival->sec = rec.isec + (float) rec.msec/1000; 
//          r->amp_v;
          arrival->amplitude = rec.amp_d; 
          arrival->period = rec.period;
          arrival->error=0.02;
          if (rec.phase[0]=='S') arrival->error=0.04;
//          arrival->quality = rec.wt;
...
//    if (!flag_gauss) {
    ...
    if (!flag_hyptype) {
        ...
        nll_putmsg(2, MsgStr);
        sprintf(MsgStr,
                "INFO: DEFAULT: \"LOCHYPOUT SAVE_NLLOC_ALL\"");
        nll_putmsg(2, MsgStr);
...
int GetNLLoc_HypOutTypes(char* line1) {
    ...
    do {
        ...
        if (strcmp(hyp_type, "SAVE_NLLOC_ALL") == 0)
            ...
            iSaveNone = 1;
            iSaveNLLocEvent = iSaveNLLocSum = 0;
            iSaveDecSec = 0;
